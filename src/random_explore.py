...
def random_grip(n_grips, pos_noise_scale=0.0):
    # Perform gripping
...
def random_roll(pos_noise_scale=0.01, roll_h_noise_scale=0.01, roll_dist_noise_scale=0.05):
    pos_noise_x = pos_noise_scale * (np.random.rand() * 2 - 1)
    pos_noise_y = pos_noise_scale * (np.random.rand() * 2 - 1)
    pos_noise_z = roll_h_noise_scale * (np.random.rand() * 2 - 1)
    start_pos = [0.4 + pos_noise_x, -0.1 + pos_noise_y, 0.2 + pos_noise_z]
    ...
    roll_dist = 0.02 + roll_dist_noise_scale * np.random.rand()
...
def random_press(pos_noise_scale=0.01, press_h_noise_scale=0.02):
    pos_noise_x = pos_noise_scale * (np.random.rand() * 2 - 1)
    pos_noise_y = pos_noise_scale * (np.random.rand() * 2 - 1)
    pos_noise_z = press_h_noise_scale * (np.random.rand() * 2 - 1)
    press_pos = [0.4 + pos_noise_x, -0.1 + pos_noise_y, 0.17 + pos_noise_z]
